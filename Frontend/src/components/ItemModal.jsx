function ItemModal({ selectedItem, setSelectedItem }) {
  if (!selectedItem) return null;

  return (
    <div
      className="fixed inset-0 bg-black/50 flex justify-center items-center z-50"
      onClick={() => setSelectedItem(null)}
    >
      <div
        className="bg-[rgba(30,41,59,0.6)] backdrop-blur-xl 
        w-[700px] max-w-[95vw] max-h-[90vh] overflow-y-auto 
        rounded-2xl p-8 
        shadow-[0_25px_50px_rgba(0,0,0,0.6)] text-white"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex justify-between mb-5">
          <h2 className="text-2xl font-bold">{selectedItem.title || selectedItem.name}</h2>
          <button onClick={() => setSelectedItem(null)} className="text-xl font-bold">✖</button>
        </div>

        {selectedItem.image && (
          <img src={selectedItem.image} alt="" className="w-full h-64 object-cover rounded-xl mb-5" />
        )}

        <div className="space-y-3 text-lg text-slate-300">
          <p><b>ID:</b> {selectedItem.id}</p>
          <p><b>Company:</b> {selectedItem.company}</p>
          <p><b>Age:</b> {selectedItem.age}</p>
          <p><b>Starting Price:</b> ₹{selectedItem.start_price || selectedItem.price}</p>
          <p><b>Condition:</b> {selectedItem.condition}</p>
          <p><b>Description:</b> {selectedItem.description}</p>
        </div>
      </div>
    </div>
  );
}

export default ItemModal;
